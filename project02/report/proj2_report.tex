...
\subsection{Preprocessing}
To clean our data, we follow an algorithm similar to Kim Yoon's paper\cite{kimyoonpaper} and code\cite{kimyooncode}. We remove duplicates on the training set and separate known contractions: for example, "don't" becomes "do n't".
...
The number of epochs can only be approximate due to all the crashes encountered on Colab but lives around 200 to 250 per model. At some point, model begins to overfit and score goes down (for the third model above this point was around 300 epochs).
...
The new techniques like BERT perform better but require lots of time and memory. They give better results than older techniques cited above but the latter achieves a relatively close 
